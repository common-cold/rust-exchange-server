...
#[tokio::test]
pub async fn place_limit_ask_then_market_bid_order() {
    ...
    assert_eq!(order1_bid.price, None);
    ...
    assert_eq!(user1_ask_balance.free_quote_qty, BigDecimal::from_str("230000000").unwrap());
    ...
    assert_eq!(user1_bid_balance.locked_quote_qty, BigDecimal::from_str("40000000").unwrap());
    ...
    assert_eq!(&bid1_ask1_trade.price, order1_ask.price.as_ref().unwrap());
    ...
    assert_eq!(&bid1_ask2_trade.price, order2_ask.price.as_ref().unwrap());
    ...
    //bid
    assert_eq!(in_memory_orderbook.bids.len(), 0);
    ...
    assert_eq!(in_memory_user1_ask_balance.free_quote_qty, BigDecimal::from_str("230000000").unwrap());
